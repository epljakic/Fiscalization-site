...
/* PROVERA PRILIKOM DODAVANJA NOVOG PREDUZECA OD STRANE ADMINISTRATORA */
if (isset($_POST['ime'])) {
    ...
    if (mysqli_num_rows($result1) == 0) {
        if (mysqli_num_rows($result2) == 0) {
            $result3 = mysqli_query($con, "insert into korisnik (korisnicko_ime,lozinka,ime,prezime,kontakt_telefon,mejl_adresa,tip_korisnika,status,prva_prijava) values "
                . "('" . $_POST['kor_ime'] . "','" . $_POST['lozinka1'] . "','" . $_POST['ime'] . "','" . $_POST['prezime'] . "','" . $_POST['telefon'] . "','" . $_POST['mejl_adresa'] . "','P','aktivan','0')");
            ...
            if (!$result3) {
                $poruka_preduzece = "<span style='color:red'>Neuspesan unos u bazu korisnik!</span>";
            }
            if (!$result4) {
                $poruka_preduzece = "<span style='color:red'>Neuspesan unos u bazu kupac!</span>";
            }
            if ($result3 && $result4) {
                $poruka_preduzece = "<span style='color:red'>Preduzece uspesno dodato!</span>";
            }
            mysqli_close($con);
        } else {
            $poruka_preduzece = "<span style='color:red'> I-mejl adresa je u upotrebi! </span>";
        }
    } else {
        $poruka_preduzece = "<span style='color:red'> Korisnicko ime je u upotrebi! </span>";
    }
}
